...
check("launch: minimalist headline is present", indexHtml.indexOf("Check your bank charges.") >= 0);
check("launch: primary CTA uses statement-first language", indexHtml.indexOf("Check my statement") >= 0);
check("launch: browser-only privacy remains explicit", indexHtml.indexOf("without uploading your file") >= 0 && indexHtml.indexOf("statement file is never uploaded or shared") >= 0);
...
check("launch: build 84 markers stay aligned", /var APP_BUILD = 84/.test(appJs) && (indexHtml.match(/\?v=84/g) || []).length === 16 && indexHtml.indexOf("?v=83") < 0);

check("launch: live checkout is enabled", indexHtml.indexOf('data-payments-live="true"') !== -1);

check("word: UI downloads DOCX from edited preview", indexHtml.includes("Download Word (.docx)") && indexHtml.includes("js/word-export.js?v=84") && appJs.includes('window.BSA_WORD_EXPORT.toBlob($("#letter-text").value)') && appJs.includes("refund_demand_letter.docx") && !appJs.includes("refund_demand_letter.txt") && appJs.includes("content instanceof Blob ? content"));
...
var smeLockedHtml = REPORT.renderSmeDashboard(smeTxns, smeAudit, { premiumUnlocked: false });
var smeUnlockedHtml = REPORT.renderSmeDashboard(smeTxns, smeAudit, { premiumUnlocked: true });
var smeMonthly = REPORT.monthlySmeReport(smeTxns, smeAudit, CTX_CURRENT, { fileName: "demo.csv" });
var smeWhatsapp = REPORT.whatsappSmeSummary(smeTxns, smeAudit);
check("SME Premium locks report exports before access", smeLockedHtml.indexOf("PREMIUM") !== -1 && smeLockedHtml.indexOf("Unlock SME Premium") !== -1 && smeLockedHtml.indexOf('btn-sme-monthly-report\" type=\"button\" disabled') !== -1);
...
check("static: APP_BUILD and cache bust agree on 84", appJs.indexOf("APP_BUILD = 84") !== -1 && (indexHtml.match(/v=84/g) || []).length >= 8 && indexHtml.indexOf("v=83") === -1);
...
check("static: review summary card leads with review amount", reportJs.indexOf('card("review", "Potential refund — review", fmtN(s.underReview || 0)') !== -1 && reportJs.indexOf("charge line(s) the auditor refuses to guess about") !== -1);
...
check("analytics: client is loaded and cache-busted", indexHtml.indexOf('js/analytics.js?v=84') !== -1 && analyticsJs.indexOf('BSA_ANALYTICS') !== -1 && analyticsJs.indexOf('/api/analytics') !== -1);
