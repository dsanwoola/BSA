...
check("launch: build 85 markers stay aligned", /var APP_BUILD = 85/.test(appJs) && (indexHtml.match(/\?v=85/g) || []).length === 16 && indexHtml.indexOf("?v=84") < 0);

check("launch: live checkout is enabled", indexHtml.indexOf('data-payments-live="true"') !== -1);

check("word: UI downloads DOCX from edited preview", indexHtml.includes("Download Word (.docx)") && indexHtml.includes("js/word-export.js?v=85") && appJs.includes('window.BSA_WORD_EXPORT.toBlob($("#letter-text").value)') && appJs.includes("refund_demand_letter.docx") && !appJs.includes("refund_demand_letter.txt") && appJs.includes("content instanceof Blob ? content"));
...
check("static: APP_BUILD and cache bust agree on 85", appJs.indexOf("APP_BUILD = 85") !== -1 && (indexHtml.match(/v=85/g) || []).length >= 8 && indexHtml.indexOf("v=84") === -1);
...
check("analytics: client is loaded and cache-busted", indexHtml.indexOf('js/analytics.js?v=85') !== -1 && analyticsJs.indexOf('BSA_ANALYTICS') !== -1 && analyticsJs.indexOf('/api/analytics') !== -1);
